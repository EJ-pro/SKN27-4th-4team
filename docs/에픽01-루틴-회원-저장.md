...
**본 에픽:** RoutinePage 게스트 동작은 유지한다. `deviceUuid.js`는 에픽01에서 **생성 권장**이며, 에픽02 통합 테스트 전에 적용해 두는 것이 좋다.
...
**주의:** `WeeklyScheduler.user` FK 필드명은 Django ORM에서 `scheduler.user_id = actor.user_id`로 assign 가능.

---

## 5. 프론트엔드 구현 — 타이핑 순서

### 5-1. `frontend/src/utils/deviceUuid.js` (권장, 에픽02 전제)

쿠키(ConsultPage)와 localStorage(RoutinePage)를 **양방향 동기화**한다. 기존 값이 한쪽에만 있으면 다른 쪽에 복사한다. **이미 양쪽에 다른 값이 있으면 덮어쓰지 않음** — 부록 A 참고.
...
기존 마운트 `useEffect` 안의 루틴 fetch(약 931행)를 함수로 분리하고 `credentials`를 추가한다.
...
ConsultPage 542행 패턴을 참고해 **상단 `WEEKLY ROUTINE OVERVIEW` 카드** 우측(「루틴 다시 설계하기」 버튼 인근)에 배치:

```jsx
{/* 상단 요약 카드 내부 — 제목 영역과 버튼 사이 또는 버튼 옆 */}
<button
...
| **1** | `actor_service.py` 생성 | import 오류 없음, 에픽2 확장 여지 확보 |
| **2** | `RoutineView` GET/POST 수정 | §8-1·8-2 curl + §8-3 소유권 격리 |
| **3** | `deviceUuid.js` (권장) | 쿠키·localStorage 동일 UUID |
| **4** | `RoutinePage` credentials + getMe UI + 재조회 | §8-4 체크리스트 |
...
4. **Navbar vs RoutinePage 표시 불일치**  
   Navbar는 마운트 시 `getMe()` 1회. RoutinePage도 동일. 로그인 직후 같은 페이지에 머물면 §5-2 Step 5 재조회로 루틴 데이터만 보완한다. Navbar 갱신은 페이지 이동·새로고침 시 자연스럽게 맞춰진다.
...
- [ ] `actor_service.py` 존재 — 에픽02에서 import 가능
- [ ] (권장) `deviceUuid.js` 적용 — 에픽02 착수 전
