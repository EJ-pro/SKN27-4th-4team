...
**다음 에픽:** [에픽03-JWT-LLM-게이트.md](에픽03-JWT-LLM-게이트.md) · 시리즈: [에픽05](에픽05-dev-머지-및-동작-수정.md) · [에픽06](에픽06-폴더-구조-및-경로-정리.md)

**구현 상태:** ✅ 완료 (2026-06-07) — 백엔드·프론트 코드 반영 완료. `backend/scripts/epic02_smoke_test.py` 스모크 PASS. §8 브라우저 수동 검증은 팀별 진행.
...
- 로그인 페이지: [`frontend/src/pages/LoginPage.jsx`](../frontend/src/pages/LoginPage.jsx)
...
- 스모크(신규): [`backend/scripts/epic02_smoke_test.py`](../backend/scripts/epic02_smoke_test.py)
...
| 항목 | 상태 |
|------|------|
| `SessionListView` GET/POST — `resolve_actor` + `session_owner_filter` | ✅ |
| `SessionDetailView` PATCH/DELETE — `session_belongs_to_actor` | ✅ |
| `MessageListView` GET/POST — actor 소유권 검증 | ✅ |
| `LoginView` — 채팅·루틴 마이그레이션 + `migrated_*` 응답 | ✅ |
| `ConsultPage` — `getMe`·닉네임 UI·`credentials`·`loadSessions` | ✅ |
| `LoginPage` — `device_uuid` body 전달 | ✅ |
| `auth.js` — `login({ device_uuid })` optional | ✅ |
...
`loadSessions`는 마운트용 `useEffect(() => { loadSessions() }, [])`와 분리한다. [에픽01](에픽01-루틴-회원-저장.md) `RoutinePage`의 `loadWeeklyRoutine` + `[authUser]` 패턴과 동일.
...
### 8-0. 자동 스모크 (`epic02_smoke_test.py`)

백엔드 컨테이너 기동 후:

```bash
docker compose up -d db backend
docker exec routinegraph_backend python scripts/epic02_smoke_test.py
```

검증 흐름: 게스트 세션 생성 → 로그인(`device_uuid`) → `migrated_sessions >= 1` → 회원 목록 조회 → DELETE(actor) → 삭제 확인.
...
| Step | 작업 | 상태 |
|------|------|------|
| **1** | `session_migration_service.py` | ✅ |
| **2** | `LoginView` + `auth.js` device_uuid | ✅ |
| **3** | `actor_service` 세션 헬퍼 추가 | ✅ |
| **4** | `SessionListView` / `Detail` / `MessageListView` | ✅ |
| **5** | `ConsultPage` credentials + getMe + UI | ✅ |
| **6** | `LoginPage` device_uuid 전달 | ✅ |
| **7** | `migrate_guest_routines_to_user` + LoginView 연동 | ✅ |
| **8** | `epic02_smoke_test.py` + [에픽01](에픽01-루틴-회원-저장.md) §8-6 | 스모크 ✅ / 브라우저 E2E 팀별 |
