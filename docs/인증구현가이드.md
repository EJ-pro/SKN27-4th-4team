...
- 대신 **서비스 레이어**에서 `AppUser.objects.get(nickname=...)` + `check_password()`로 검증한다.
...
### 4-2. `backend/api/auth_service.py` (신규 생성)

인증 비즈니스 로직을 views와 분리한다. views는 HTTP만, 검증·DB·JWT는 여기서 처리한다.
...
| Day | 작업 | 완료 기준 |
|-----|------|-----------|
| **Day 1** | `requirements.txt` + `auth_service.py` + `RegisterView` + urls | curl register 201 |
